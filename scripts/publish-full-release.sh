...
# 产物:
#   dist/DiandianMini.dmg          → GitHub release vYYYY.MM.DD  (首次安装用)
...
# ── 3. 打 DMG ─────────────────────────────────────────────────────────────────
DMG_PATH="dist/DiandianMini.dmg"
rm -f "$DMG_PATH"
echo "▶ 正在制作 DMG…"
hdiutil create \
...
gh release create "$VERSION_TAG" \
  ...
  --title "DiandianMini $VERSION_TAG" \
  --notes "$(printf 'codever:%s\nchangelog:%s\n\n首次使用请下载 DiandianMini.dmg 安装。已安装用户会通过热更新自动升级，无需重新下载。' "$CODEVER" "$CHANGELOG")" \
  "${DMG_PATH}#DiandianMini.dmg"

echo "   ✅ 完整包 release: https://github.com/371066607/diandian-mini/releases/tag/$VERSION_TAG"
...
echo ""
echo "✅ 发布完成"
echo "   完整包 : https://github.com/371066607/diandian-mini/releases/tag/$VERSION_TAG"
echo "   热更新 : https://github.com/371066607/diandian-mini/releases/tag/code"
